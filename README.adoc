...
`jsongrep`), but those consumed memory very much when I fed them a
...
//https://repo1.maven.org/maven2/org/meeuw/mihxil-json/{version}/mihxil-json-{version}-all.tar.gz[here].
https://repo1.maven.org/maven2/org/meeuw/mihxil-json/{version}/mihxil-json-{version}-all.zip[here].

This zip also contains executable scripts to call them with `java -jar`, which will work in a unix or osx environment and can be unzipped somewhere in your path. Typing this installs in the current directory:
...
----
//curl -fsSL https://repo1.maven.org/maven2/org/meeuw/mihxil-json/{version}/mihxil-json-{version}-all.tar.gz | tar -xz

As a library, please refer to link:https://central.sonatype.com/search?q=g%3Aorg.meeuw+json[maven central].


|===
|Version | Jackson | Date | Notes

| 1.1
| 3
| 2026-09
| Less checked exceptions, more following jackson3, ignore case command line arguments
...
This just demonstrates a simple path match. It returns the matched path together with the associated value.
...
Generally, the available options are documented in the tools themselves too
...
y.arr[0].d=y
----
This is useful for array indices. But you can also choose it completely ignores array indices in matching, which may simplify things:
...
Another example on a couchdb database (finds documents where a certain field has a certain value)
...
Normally, when using this 'record' functionality, the output record will be implicitly sorted like the matches. So in this case first the 'mid', then 'publishDate' then 'lastModified', independent of the order they appeared in the JSON document. With the `-sortfields` parameter you can disable this behavior and output in the original order.
