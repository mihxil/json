...
This just demonstrate a simple path patch. It returns the matched math together with the associated value.
...
Regex matching on key is also possible, which can e.g. be used to output different keys at the same level.
